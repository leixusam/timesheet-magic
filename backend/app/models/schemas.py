...
class FinalAnalysisReport(BaseModel):
    ...
    # Task 9.6: Two-pass processing metadata
    processing_metadata: Optional[Dict[str, Any]] = Field(None, description="Metadata about the processing approach used (single-pass vs two-pass) and performance metrics.")
    processing_mode: Optional[str] = Field(None, description="Processing mode used: 'single_pass', 'two_pass', or 'fallback_to_single_pass'.")
    discovered_employees: Optional[int] = Field(None, description="Number of employees discovered in two-pass mode (Pass 1).")
    quality_score: Optional[float] = Field(None, description="Two-pass processing quality score (0-100%) if applicable.") 